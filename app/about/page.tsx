export default function About(){
 ...
 return <main className="subpage"><div className="container">
  ...
  <section className="about-hero">
   ...
   <h1>I’m building traders around <span>process, discipline and long-term thinking.</span></h1>
   <p>I’m the founder of Institutional Trading Academy. I did not become consistent overnight. My journey taught me that becoming profitable is a process that requires time, testing, discipline and the ability to survive your mistakes.</p>
   <div className="profit-timeline"><div className="profit-number">12</div><div><strong>MONTHS+</strong><span>A realistic horizon to develop and validate a profitable trading process.</span></div></div>
   <p>The goal of ITA is not to promise easy money. It is to teach a framework you can study, test, journal and execute consistently.</p>
   <a className="button primary" href="/#offers">Explore the Programs</a>
  </section>
  <section className="story-section">
   <div className="story-heading"><div className="eyebrow">THE JOURNEY</div><h2>What I went through before becoming profitable.</h2><p>These are the major stages of my trading journey — including the failures, withdrawals, strategy changes and testing periods.</p></div>
   <div className="timeline">{milestones.map(([date,title,text])=><article className="timeline-item" key={date+title}><div className="timeline-date">{date}</div><div className="timeline-content"><h3>{title}</h3><p>{text}</p></div></article>)}</div>
  </section>
  <section className="proof-section"><div className="story-heading"><div className="eyebrow">PROOF OF THE JOURNEY</div><h2>Milestones, certificates & withdrawals.</h2><p>Four spaces are reserved for certificates, challenge results or payout screenshots.</p></div>
   <div className="proof-grid">{["Challenge Certificate","Payout Proof","Funding Certificate","Trading Milestone"].map((label,i)=><div className="proof-slot" key={label}><div className="proof-slot-number">0{i+1}</div><div className="proof-placeholder"><span>ADD IMAGE</span><strong>{label}</strong><small>Replace this slot with your certificate or screenshot.</small></div></div>)}</div>
  </section>
